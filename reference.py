...
def ray_trace(n, angles, init_dir, start, curvature, ray_path, ray_dir, transmittance):
    ...
    midpt = size - diff / 2, 0.5
    # Line-Sphere Intersection
    lens_radius = diameter / (2 * curvature)
    rs = np.sqrt(lens_radius ** 2 - diameter ** 2 / np.float32(4))
    c = midpt[0] + norm[0] * rs, midpt[1] + norm[1] * rs
    o = ray_path[-2]
    l = ray_dir[-2]
    under = (l[0]*(o[0] - c[0]) + l[1]*(o[1] - c[1]))**2 - ((o[0] - c[0])**2+(o[1] - c[1])**2) + lens_radius ** 2
    ...
    assert ((x[0] - midpt[0])**2+(x[1] - midpt[1])**2) <= diameter ** 2 / 4, "ray doesn't intersect lens surface"
