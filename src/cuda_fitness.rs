...
use crate::ray::{CompoundPrism, DetectorArray, GaussianBeam, DetectorArrayPositioning, Spectrometer};
...
use rustacuda::{quick_init, launch};
use rustacuda::memory::{cuda_malloc, DeviceBox, DeviceBuffer, DeviceCopy};
...
const PTX: &[u8] = concat!(include_str!("../target/nvptx64-nvidia-cuda/release/compound_prism_designer.ptx"), "\0").as_bytes();
...
const NWARP: u32 = 2;
const SEEDS: &[f64] = &[0.4993455843,0.8171187913, 0.8234955201,0.3911129692,
    0.2887278677,0.5004948416, 0.6997901652,0.1544760953, 0.8681075152,0.3130246465,
    0.0472957030, 0.5421335682, 0.5607025594, 0.5911739281, 0.7870769069];
...
impl CudaFitnessContext {
    ...
    fn launch_p_dets_l_ws<'a>(&mut self, seed: f64, spec: &Spectrometer<'a>) -> rustacuda::error::CudaResult<Vec<f64>> {
        ContextStack::push(&self.ctxt)?;

        let beam = &spec.gaussian_beam;
        let cmpnd = &spec.compound_prism;
        let detarr = &spec.detector_array;
        let detpos = &spec.detector_array_position;
        self.dev_spec.copy_from(unsafe { std::mem::transmute::<&Spectrometer<'a>, &Spectrometer<'static>>(spec) })?; // FIXME immediately, very unsafe & dangerous
        ...
        let dev_probs = match &mut self.dev_probs {
            Some(ref mut dev_probs) if dev_probs.len() == MAX_N * detarr.bins.len() => {
                dev_probs
            }
            _ => {
        ...
        let function = self.module.get_function(unsafe { CStr::from_bytes_with_nul_unchecked(FNAME) })?;
...
pub fn set_cached_cuda_context(ctxt: Context) -> rustacuda::error::CudaResult<()> {
    if let Some(mutex) = CACHED_CUDA_FITNESS_CONTEXT.get() {
        ...
        CACHED_CUDA_FITNESS_CONTEXT.get_or_try_init(|| CudaFitnessContext::new(ctxt).map(Mutex::new))?;
    }
    Ok(())
}
...
impl<'a> Spectrometer<'a> {
    pub fn cuda_fitness(&self) -> Result<DesignFitness, CudaFitnessError> {
        ...
        let mutex = CACHED_CUDA_FITNESS_CONTEXT.get_or_try_init(|| CudaFitnessContext::new(quick_init()?).map(Mutex::new))?;
        ...
        for &seed in SEEDS {
            ...
            for p_dets_l_w in p_dets_l_ws.chunks_exact(detarr.bins.len()) {
                for ((p_det, stat), p) in p_dets.iter_mut()
        ...
        let errors: Vec<_> = p_dets.iter().map(|s| s.sem()).filter(|e| e >= &MAX_ERR).collect();
        eprintln!("cuda_fitness error values too large (>={}), consider raising max wavelength count: {:.3?}", MAX_ERR, errors);
        return Err(CudaFitnessError::IntegrationAccuracyIssue);
    }
}
