...
pub fn new_catalog<N: RealField + FromStr>(
    file: &str,
) -> Result<BTreeMap<String, Glass<N>>, CatalogErr> {
    ...
    for line in file.lines() {
        if line.starts_with("NM") {
            ...
            if let Some(_) = name.replace(nm.get(1).ok_or(CatalogErr::NameNotFound)?.to_uppercase())
            {
                return Err(CatalogErr::GlassDescriptionNotFound);
            }
            dispform = nm
                .get(2)
                .and_then(|d| d.parse().ok())
                .ok_or(CatalogErr::GlassTypeNotFound)?;
