...
use cpython::{
    Python, PyResult, ToPyObject, FromPyObject, PyObject,
    PyTuple, PythonObjectWithTypeObject, ObjectProtocol,
};
mod glasscat;
mod quad;
mod ray;
...
use crate::ray::{merit, trace, transmission, get_spectrometer_position, Prism, PmtArray, GaussianBeam};
...
py_module_initializer!(prism, initprism, PyInit_prism, |py, m| {
    ...
    let prism_tuple = namedtuple.call(py, ("Prism", "glasses, angles, curvature, height, width"), None)?;
    ...
    m.add(py, "create_catalog", py_fn!(py, create_catalog(file: &str) -> PyResult<impl ToPyObject> {
            to_pyresult(py, new_catalog::<f64>(file))
    }))?;
    m.add(py, "fitness", py_fn!(py, fitness(wavelengths: Vec<f64>, prism: OwnedPrism<f64>, pmts: OwnedPmtArray<f64>, beam: GaussianBeam<f64>) -> PyResult<impl ToPyObject<ObjectType = PyTuple>> {
        to_pyresult(py, py.allow_threads(|| merit(&wavelengths, (&prism).into(), (&pmts).into(), beam).map(|[size, info]| (size, info))))
    }))?;
